...
- 如果本次提交不包含 Rust 文件，hook 会直接跳过。
- 如果本次提交包含 Rust 文件，hook 会使用 `rustfmt` 只整理本次提交涉及且当前仍存在的 Rust 文件。
- 如果 `rustfmt` 改写了文件，hook 会阻止提交，并提示你检查 diff、重新暂存后再提交。
- 如果 `rustfmt` 执行失败，hook 也会阻止提交，并输出错误信息。

### Stop

当 Claude 完成一轮响应时：

- hook 会先判断当前工作区里是否存在本地改动的 Rust 文件。
- 只要存在这类改动，hook 就会进一步执行 `cargo fmt --all --check` 做工作区级格式检查。
- 如果检查失败，会给出提醒。
- 这个提醒不会阻塞后续操作。
