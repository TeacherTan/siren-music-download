...
const includesAllTrackedChanges = /(?:^|\s)(?:-a|--all)(?=\s|$)/u.test(hookCommand);
const rustFiles = includesAllTrackedChanges
  ? [...new Set([...getStagedRustFiles(repoRoot), ...getTrackedChangedRustFiles(repoRoot)])]
...
const targetRustFiles = [...new Set(rustFiles)].sort((left, right) => left.localeCompare(right));
...
if (changedFiles.length > 0) {
  emitPreToolBlock({
    ...
    permissionDecisionReason: "rustfmt changed Rust files; review and restage before committing.",
  });
}
